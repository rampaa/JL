    ...
    public static void InsertToKanjidicDB(Dict dict)
    {
        ...
        foreach ((string kanji, IList<IDictRecord> records) in dict.Contents)
        {
            foreach (IDictRecord record in records)
            {
                ...
                _ = insertRecordCommand.Parameters.AddWithValue("@on_readings", kanjidicRecord.OnReadings is not null ? JsonSerializer.Serialize(kanjidicRecord.OnReadings, Utils.s_jsoWithIndentation) : DBNull.Value);
                _ = insertRecordCommand.Parameters.AddWithValue("@kun_readings", kanjidicRecord.KunReadings is not null ? JsonSerializer.Serialize(kanjidicRecord.KunReadings, Utils.s_jsoWithIndentation) : DBNull.Value);
                _ = insertRecordCommand.Parameters.AddWithValue("@on_readings", kanjidicRecord.OnReadings is not null ? JsonSerializer.Serialize(kanjidicRecord.OnReadings, Utils.s_jsoWithIndentation) : DBNull.Value);
                _ = insertRecordCommand.Parameters.AddWithValue("@nanori_readings", kanjidicRecord.NanoriReadings is not null ? JsonSerializer.Serialize(kanjidicRecord.NanoriReadings, Utils.s_jsoWithIndentation) : DBNull.Value);
                _ = insertRecordCommand.Parameters.AddWithValue("@radical_names", kanjidicRecord.RadicalNames is not null ? JsonSerializer.Serialize(kanjidicRecord.RadicalNames, Utils.s_jsoWithIndentation) : DBNull.Value);
                _ = insertRecordCommand.Parameters.AddWithValue("@glossary", kanjidicRecord.Definitions is not null ? JsonSerializer.Serialize(kanjidicRecord.Definitions, Utils.s_jsoWithIndentation) : DBNull.Value);
    ...
    public static List<IDictRecord> GetRecordsFromKanjidicDB(string dbName, string term)
    {
        ...
        while (dataReader.Read())
        {
            ...
            string[]? nanoriReadings = nanoriReadingsFromDB is not DBNull
                ? JsonSerializer.Deserialize<string[]>((string)nanoriReadingsFromDB, Utils.s_jsoWithIndentation)
                : null;

            object radicalNamesFromDB = dataReader["radicalNames"];
            string[]? radicalNames = radicalNamesFromDB is not DBNull
                ? JsonSerializer.Deserialize<string[]>((string)radicalNamesFromDB, Utils.s_jsoWithIndentation)
                : null;
